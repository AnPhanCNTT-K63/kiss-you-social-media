...
import { handlerBetweenOperator } from './search-operator/handler-between-operator.utils';
import { handlerComparisonOperator } from './search-operator/handler-comparison-operator.utils';
import { handlerInAndNotInOperator } from './search-operator/handler-in-and-not-in-operator.utils';
...
export const createSearchPipeline = (search: any, searchType: string) => {
  ...
  const result = keys
    .map((key) => {
      ...
      if (!operator) {
        throw new UnprocessableEntityException('Invalid search query operator');
      }
  ...
  result.forEach((item) => {
    ...
    addSearchConditionToPipeline(pipeline, operator, field, value, searchType);
  });
