"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ShareService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("mongoose");
const share_entity_1 = require("./entities/share.entity");
const mongoose_2 = require("@nestjs/mongoose");
const post_service_1 = require("../post/post.service");
const notification_service_1 = require("../notification/notification.service");
let ShareService = class ShareService {
    constructor(shareModel, postService, notificationService) {
        this.shareModel = shareModel;
        this.postService = postService;
        this.notificationService = notificationService;
    }
    async create(shareDto, user) {
        try {
            await this.shareModel.create({
                post: new mongoose_1.Types.ObjectId(shareDto.post),
                user: user._id,
            });
            const existingPost = await this.postService.postModel.findOne({
                _id: shareDto.post,
            });
            existingPost.shares += 1;
            await existingPost.save();
            await this.postService.postModel.create({
                createdBy: user._id,
                content: shareDto.content,
                sharedPost: existingPost._id,
                isApproved: true,
            });
            await this.notificationService.create(shareDto.notification, user._id);
        }
        catch (error) {
            throw new common_1.BadRequestException(error);
        }
    }
    async count(postId) {
        try {
            return (await this.shareModel.find({ post: new mongoose_1.Types.ObjectId(postId) }))
                .length;
        }
        catch (error) {
            throw new common_1.BadRequestException(error);
        }
    }
};
exports.ShareService = ShareService;
exports.ShareService = ShareService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_2.InjectModel)(share_entity_1.Share.name)),
    __metadata("design:paramtypes", [mongoose_1.Model,
        post_service_1.PostService,
        notification_service_1.NotificationService])
], ShareService);
//# sourceMappingURL=share.service.js.map